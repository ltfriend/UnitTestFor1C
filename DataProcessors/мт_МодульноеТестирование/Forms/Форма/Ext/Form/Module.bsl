...
Процедура ПостроитьДеревоТестов()
	...
	|			ТОГДА 0
	|		КОГДА МИНИМУМ(ТестПройден) = ИСТИНА
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ КАК КартинкаРезультата,
...
Процедура ВыполнитьТестыГруппы(СтрокиДерева, РезультатыТестов)
	...
	РезультатыТестов = Новый Структура("Длительность, БылиОшибки", 0, Ложь);
	
	Для каждого СтрокаДерева Из СтрокиДерева Цикл
		
		Если СтрокаДерева.ЭтоГруппа Тогда
			
			Если СтрокаДерева.КартинкаРезультата = 3 Тогда
				СтрокаДерева.КартинкаРезультата = 4;
				ЭтаФорма.ОбновитьОтображениеДанных();
			КонецЕсли; 
			
			РезультатыТестовГруппы = Неопределено;
			ВыполнитьТестыГруппы(СтрокаДерева.ПолучитьЭлементы(), РезультатыТестовГруппы);
			
			Если СтрокаДерева.КартинкаРезультата = 4 Тогда
				СтрокаДерева.ТестПройден = Не РезультатыТестовГруппы.БылиОшибки;
				СтрокаДерева.КартинкаРезультата = ?(СтрокаДерева.ТестПройден, 1, 2);
				СтрокаДерева.ДатаВыполнения = ТекущаяДата();
				СтрокаДерева.Длительность = РезультатыТестовГруппы.Длительность;
				...
				РезультатыТестов.Длительность = РезультатыТестов.Длительность +
													РезультатыТестовГруппы.Длительность;
				Если РезультатыТестовГруппы.БылиОшибки Тогда
		...
		Иначе
			// В остальных случаях обработка строки не производится.
...
Функция ПолучитьТестыДляВыполнения(ТипТеста)
	...
	Для каждого СтрокаТеста Из ТаблицаТестов Цикл
		...
		Если ТестПодходитДляВыполнения(СтрокаТеста, ТипТеста) Тогда
...
&НаКлиенте
Функция ТестПодходитДляВыполнения(СтрокаТеста, ТипТеста)
...
Функция ПредставлениеДлительности(Длительность)
	...
	ИначеЕсли Длительность < ДлительностьМинута Тогда
		...
		Возврат СтрШаблон(Длительность / 1000, "ЧДЦ=1") + " " + НСтр("ru='с'");
