...
Перед создание первого теста необходимо поместить в подключенное расширение
объект, для функций и процедур которого создаются тесты. Например, для написания
тестов общего модуля "ДоставкаТоваров" в расширение необходимо поместить в
расширение этот общий модуль. А для возможности создать тест для процедуры
...
Для того, чтобы иметь возможность вызывать созданную процедуру, её необходимо
добавить в список тестовых процедур. Для этого в том же модуле, где создаются
тестовые процедуры необходимо добавить экспортную процедуру
"тест_ДобавитьТестыВСписок(СписокТестов)". В этой процедуре необходимо добавить
все ваши тестовые процедуры с использованием функции
...
Пример тестовой процедуры с параметром:
	...
	Процедура тест_УправлениеДоступностью_ЗаполнениеСклад_ПроверкаДоступностиСтраниц(Параметры) Экспорт
		...
		// Проверка
		мт_ТестированиеКлиентСервер.ПроверитьЗначенияРавны(Параметры.Доступность, Элементы.Страницы.Доступность);
...
Пример добавления тестовой процедуры с параметрами в список тестов.
	...
	Процедура тест_ДобавитьТестыВСписок(СписокТестов) Экспорт
		...
		Параметры = Новый Структура("Склад, Доступность",
									КакойТоСклад, Истина);
...
Пример процедуры для тестирования HTTP-сервиса.
	...
	Процедура тест_ПриходныйОрдерПоШтрихкодуGET_ДокументНайден() Экспорт
		...
		Если Не ДанныеДокумента.Следующий() Тогда
			ВызватьИсключение НСтр("ru='В системе отсутствуют приходные ордера. Выполнить тест невозможно.'");
		КонецЕсли; 
		...
		Если Не (ПолученныйНомер = ДанныеДокумента.Номер
					И ПолученнаяДата = ДанныеДокумента.Дата)
		Тогда
...
2. В списке расширений выбрать нужно расширение и выполнить команду **Конфигурация / Сравнить, объединить с конфигурацией из файла**
в командной панели формы списка расширений.
3. Выбрать расширение с библиотекой тестирования.
4. В окне сравнения, объединения снять флаг с корня дерева, выбрать в меню команду **Действия / Отметить по подсистемам файла"**
и выбрать две подсистемы: **мт_МодульноеТестирование** и **мт_ТестируемыеОбъекты**.
