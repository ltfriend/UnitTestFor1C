...
// Выполняет сравение значений свойств структуры с эталлонной. Если значение свойства эталонной структуры отличается от
// значения сравниваемой структуры или свойство вообще отсутствует, то вызывается исключение.
//
// Параметры:
//  ЭталоннаяСтруктура - Структура - эталонная структура, с которой будут сравниваться значения свойств.
//  СравниваемаяСтруктура - Структуру - структура, значения свойств которой будут сравниваться с эталонной структурой.
//
Процедура ПроверитьСтруктурыИдентичны(ЭталоннаяСтруктура, СравниваемаяСтруктура) Экспорт
	
	Если Не СтруктураИдентичный(ЭталоннаяСтруктура, СравниваемаяСтруктура) Тогда
		ВызватьИсключение НСтр("ru='Значения структуры отличаются от эталонной.'");
	КонецЕсли; 
	
КонецПроцедуры 

// Выполняет сравение значений свойств структуры с эталлонной. Если значение свойства эталонной структуры полностью
// совпадают со значениями сравниваемой структуры, то вызывается исключение.
//
// Параметры:
//  ЭталоннаяСтруктура - Структура - эталонная структура, с которой будут сравниваться значения свойств.
//  СравниваемаяСтруктура - Структуру - структура, значения свойств которой будут сравниваться с эталонной структурой.
//
Процедура ПроверитьСтруктурыРазличны(ЭталоннаяСтруктура, СравниваемаяСтруктура) Экспорт
	
	Если СтруктураИдентичный(ЭталоннаяСтруктура, СравниваемаяСтруктура) Тогда
		ВызватьИсключение НСтр("ru='Значения структуры идентичны эталонной.'");
	КонецЕсли; 
...
Функция СтруктураИдентичный(ЭталоннаяСтруктура, СравниваемаяСтруктура)
	Перем СравниваемоеЗначение;
	
	Для каждого ЭлементЛевогоЗначения Из ЭталоннаяСтруктура Цикл
		
		Если Не СравниваемаяСтруктура.Свойство(ЭлементЛевогоЗначения.Ключ, СравниваемоеЗначение)
				Или ЭлементЛевогоЗначения.Значение <> СравниваемоеЗначение
		Тогда
			Возврат Ложь;
		КонецЕсли; 
