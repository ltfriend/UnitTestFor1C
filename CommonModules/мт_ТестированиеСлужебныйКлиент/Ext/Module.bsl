...
Функция НастройкиТестированияПоУмолчанию() Экспорт
	...
	НастройкиТестирования.Вставить("КаталогЖурнала", КаталогДокументов() + "1cunit\");
	...
	НастройкиТестирования.Вставить("ПробелыВместоПодчеркиванияВНазванияхТестов", Истина);
	НастройкиТестирования.Вставить("УдалятьПрефиксВНазванияхТестов", Истина);
	НастройкиТестирования.Вставить("ПрефиксНазванийТестов", "тест_");
...
Процедура ПрочитатьНастройкиТестированияИВыполнитьАвтозапускТестов() Экспорт
	...
	Если ТипЗнч(глНастройкиТестирования) <> Тип("Структура") Тогда
		глНастройкиТестирования = НастройкиТестированияПоУмолчанию();
	КонецЕсли; 
...
// Возвращает полный путь к файлу, в котором сохраняются результаты выполнения тестов.
//
// Параметры:
//  КаталогРезультатов - Строка - путь к каталогу, в котором сохраняются результаты тестов.
// 
// Возвращаемое значение:
//  Строка - полный путь к файлу, в котором сохраняются результаты выполнения тестов.
//
